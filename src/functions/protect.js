...
async function protect(msg) {
    ...
    if (array[0]) {
        if (msg.mentions.users.first()) {
            return await memeHalf(msg.mentions.users.first().avatarURL, './src/imgs/protect.png', 3);
        }
        if (array[0].match(regex) != null) {
            return await memeHalf(array[0], './src/imgs/protect.png', 3);
        }
    }
    if (msg.attachments.size > 0) {
        if (msg.attachments.first().url.match(regex) != null) {
            return await memeHalf(msg.attachments.first().url, './src/imgs/protect.png', 3);
        }
    }
    return await memeHalf(msg.author.avatarURL, './src/imgs/protect.png', 3);
}
